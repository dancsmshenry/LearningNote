...
- **上下文**是由程序正确运行所需的状态组成的，这个状态包括存放在内存中的程序的代码和数据，它的栈、通用目的寄存器的内容、程序计数器、环境变量以及打开文件描述符的集合
- 进程提供给应用程序的关键抽象：
  - 一个独立的逻辑控制流
  - 一个私有的地址空间
...
- <img src="images/进程地址空间.png" style="zoom:150%;" />
...
- 没有设置模式位时，进程就运行在**用户模式**中，用户模式中的进程不允许执行特权指令
  - 比如停止处理器、改变模式位，或者发起一个I/O操作
