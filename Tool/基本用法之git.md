...
```shell
git clone <url>				# clone 指定仓库的代码
git clone <url> <dir_name>	# 将代码 clone 到指定目录中
```
...
- ```shell
  ...
  git reset HEAD	#	可以用于将暂存区中所有的内容都撤销回工作区
  git reset HEAD filename	#	将某个文件撤销回工作区
  
  git reset --soft HEAD^ #一般用于撤销本地仓库中未提交（到远程仓库）的 commit
  git reset --soft commitId # 和上面一样（不过需要先用 git log 查看此前 commit 的 id）
  #用上述命令撤销后，修改的内容会回到工作区
  ```
...
```shell
git pull <remote> <remote_branch>:<local_branch>			# 相当于 fetch + merge
git pull --rebase <remote> <remote_branch>:<local_branch>	# 相当于 fetch + rebase
```
