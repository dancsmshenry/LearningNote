...
- 方式一：v存储的是一整行的数据（early materialization，尽早的物化数据）
  - 数据排序后不需要回表查询
- 方式二：v存储的是记录的id或者主键值（late materialization，延迟数据的物化）
...
假设：数据集被分解为N个页面，DBMS的缓冲区有B个缓冲页来储存数据
...
<img src="image/2-way external merge sort_01.png" style="zoom: 150%;" />
...
Phase I：使用B个缓冲页，数据表总共有N个数据页，总共需要`[N/B]`次读取数据，并对其中的数据进行排序

Phase II：使用K路合并对剩余的页面进行合并
