...
应用先向主节点（primary node）发送事务开始的请求
...
主节点（primary node）需要去各个节点判断，事务是否可以在本节点上安全的提交
...
但，还没有讨论如何确保所有的节点都能够成功的提交事务

比如说，如果单个副节点的提交失败了，该如何处理

或者，节点直接的通信发生了严重的延迟（rpc调用延迟过长）

又或者，不想等待所有的节点都同意再提交，该怎么处理？
...
数据节点分布在全世界，但这里要假设每个节点的DBMS是正常工作的（非恶意的），都是可控的（是在可以管理的情况下的）

也就是说，数据节点不会被黑客攻击

<br/>

也就是说，这里的分布式数据库是暂时不考虑**拜占庭问题**的，这里假设每个数据节点都是善意的

PS：而类似区块链这种场景，可以理解为是一个分布式数据库，但是每个节点的信息是不可靠的（因为每个节点都是由不同的人或机构组成的，互相之间不可信）

因此在区块链的场景下，需要更加复杂的共识算法去实现共识机制，需要解决拜占庭问题
...
两阶段提交分为两个部分：准备阶段和提交阶段

当事务发送提交请求的时候，就会进入这两个阶段
...
首先接收到应用的commit request的节点会变为协调者（coordinator）
...
<img src="image/2PC_04.png" style="zoom:150%;" />
...
一并发送可以提前开始投票，即提前开始prepare过程

<br/>

**Early acknowledgement after prepare**

当prepare阶段，收到所有节点的同意的时候，就可以提前告知用户事务commit success

而不是等到commit阶段以后再告诉用户
...
通过投票机制，取得大部分人的共识，为了选出一个大部分人都认可的决策
...
在收到acceptor的回复以后，如果有超过一般的acceptor都同意这个提案
...
面对那些处于崩溃状态的节点（即acceptor），proposer会持续不断地向其发送信息
...
而此时又来了一个事务n+1，开始了它的第一阶段propose

而Paxos为了要实现可串行化，因此就会要将原来的事务n给拒绝掉
...
缺点就是如果这个proposer挂了的话，就会导致系统的崩溃

因此，为了维护这个proposer，系统会周期性的维护这个proposer，即每隔一段时间就会选举出一个新的proposer

这种隔一段时间选取出一个新的leader的机制在Raft中名为租约
...
而Paxos，只要大多数的参与节点不会宕机（指一半），系统就可以运作
...
主节点可以在没有原子提交的情况下（是指可以只在主节点上操作，而不用上述的多节点原子提交），通过log的方式将数据分发给备节点
...
主节点将日志传给备节点，然后当日志传输成功了以后，就返回给主节点，留着备节点慢慢执行
...
主备同步的时候，主节点传递的是SQL语句

也就是说，在主节点执行的SQL语句，还需要在副节点上再执行一遍

即SQL需要在每个节点上单独执行一遍

同时在最后，还要检查事务是否在每个副本中都得到了相同的结果

缺点：又需要在备节点上重新执行一遍SQL
...
主备同步的时候，主节点先将SQL执行一遍，记录到底需要修改那些数据，并记录成日志
...
`consistent`（一致性；也就是线性一致性），`always available`（高可用；即使部分节点崩溃，也能对外提供服务），`network partition tolerant`（网络分区容忍；如果一个集群因为网络的断开，会不会因此变为两个集群）

这三者是永远都不可能同时实现的，最多同时实现其中的两点
...
这里的一致性是强一致性，就是说数据一旦写入到了系统中

那么无论就必须能够立刻在副本或是其他分片上读到这份数据
...
两个交流的集群发生了网络切断，造成了网络分区

即每一方都认为自己没挂，是对方挂了，那么就会造成备份节点也成为了主节点的角色

<img src="image/partition tolerance_01.png" style="zoom:150%;" />

<br/>

那么此时，如果两个应用各自向主节点Primary提交修改相同数据的事务
...
即**牺牲可用性，以此保证一致性和分区容忍性**

<br/>

但是对于一些NoSQL型数据库来说，会在分裂的集群重新连接以后

判断哪些不一致的数据，选取其中一部分并保留下来
...
将多个DBMS连接在同一个分布式架构中，是一件非常困难的事情：
...
利用Pg作为中间件（服务端），连接各个数据库

<img src="image/federated database example.png" style="zoom:150%;" />
...
那么可以让一些其他种类的数据库作为备节点，让MySQL的主节点源源不断的发送log，然后在本地解析它

一个比较常用的场景，日常使用的是TP型数据库，而需要用AP型数据库对数据进行分析

那么就可以让AP型数据库作为备节点，让主节点的TP数据库不断的向它发送日志

另一个场景，就是需要检查某个数据是否发生了变化，那么就可以将将数据同步到其他的DBMS中，实现监听（或者数据库的触发器也是可以的）
...
因此需要使用更加复杂的协议来提交事务，以此实现DBMS的正确运行（比如工作量证明等）