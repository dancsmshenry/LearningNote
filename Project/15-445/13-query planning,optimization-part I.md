...
一个争论：人为的制定的执行计划和机器做出的执行计划，哪个效率会好？
...
因此，也需要知道数据的分布（需要知道算子要处理多少数据，才能够知道代价是多少）
...
把上面的抽象语法树转化成一个优化器可以工作的最原始的逻辑结构（又叫作正则化）
...
最后会生成一个物理的查询计划 physical plan，里面都是真正需要执行的算子
...
也有人用Machine Learning对优化器进行优化，比如DB2就用了人工智能优化器
...
但是这对于一个列存的数据库来说就无用了，因为它始终是最后才物化的
...
即，可以先把子查询先做了，然后把结果放到单独的一个表里面（作为子查询的结果）
...
<img src="image/decomposing queries.png" style="zoom:150%;" />
...
比如下图，1是不等于0的，所以这里可以直接优化为false，输出空结果即可
...
此前讨论的都是基于规则的优化（启发式），后续则需要讨论基于开销的优化

基于代价的优化，需要根据数据库数据的具体分布等情况
...
因为目前主要研究的是基于磁盘的DBMS，因此相比CPU的开销，磁盘IO的开销更大，更值得关注

- CPU的开销几乎可以忽略
- 必须考虑随机IO和顺序IO（同时要减少随机IO，改为顺序IO）
