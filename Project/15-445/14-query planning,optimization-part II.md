...
R代指数据表的表名；A代指数据表的某一列

**N<sub>R</sub>**：当前的数据表中有多少条数据

**V(A,R)**：在当前数据表R的某一列A中，有多少个不同类型的值（比如性别一列，就只有男女的差别）

**SC(A,R)**（全称：**selection cardinality**）：对于当前的选取方式，选取的基数是多少

- 在当前的选取方法中，平均每次选取能够得到多少个值
- 比如说在性别一列，总共有50个男性和50个女性，那么此时选取出男性的基数就是50个
- 对于这一列每一个单独的值，如果选取这个值，平均每次能获取到多少个值
- 计算公式：**N<sub>R</sub>/V(A,R)**
- 不过这种对于选取数量的衡量，前提是数据是平均分布的（均匀的）
...
在这种情况下，选取基数要么是1（存在且只存在一个），要么是0（不存在）

<img src="image/logical costs_01.png" style="zoom:150%;" />
...
选择率，当前的谓词在总的数据中，能够选取出多少个数据，这个概率便是选择率

然后可以根据每个谓词的选择率，计算当前这种方案下总体的选择率

以下的例子都假设总共有五个数据，分别是0、1、2、3、4；并且，都假设数据的分布是均匀的
...
- 针对给定的值，在当前数据表中平均能够找到**SC(P)**个
- 而总共有**N<sub>R</sub>**个数据，因此相等谓词的选择率便是**SC(P)/N<sub>R</sub>**

<img src="image/equality predicate.png" style="zoom:150%;" />
...
- 下图中A<sub>max</sub>是指当前列数据中的最大值，A<sub>min</sub>是当前列数据中的最小值
...
<img src="image/negation query.png" style="zoom:150%;" />
...
这里，可以将数据的选择率和概率作为替换，因此针对多谓词并联（**求并集**）的复合查询，就可以用概率的相乘获得选择率（因为二者的概率是互不干扰，独立的）
...
前面说的主要是语句基于谓词的查询，而如果涉及到两个表的jion，情况会更加麻烦
...
而，因为join是符合交换律的，所以可以交换两边表的位置，因此在选取最终在选取最终结果的时候

往往是选择总数据量较小的那个，也就是被除数较小的那个
...
实际上，此前做出的很多分析都是基于上述三个假设，而如果没有三个假设，就会出现问题

<img src="image/correlated attributes.png" style="zoom:150%;" />

比如这里，针对makes和models两条列，希望找到make=“honda”和model=“accord”的数据

而如果基于此前的分析，1/10 * 1/100 = 0.001即答案

而如果对数据加以了解，就会发现，model=“accord”的，就只能是make=“honda”的，那么实际上的概率应该就是1/100 = 0.01

也就是此时的推断出现了问题（主要就是两个谓词的查询之间，从数据的角度来说是不独立的）
...
- 比如说10、11、12这一块，当数据进行打包处理以后，很难知道11的位置到底有没有数据
- 也就是说，在块内的数据范围会因此被掩盖
- 因此需要引入等高直方图
...
- 即，从概率的角度来推断数据出现的数量以及概率

主要有以下两个方法：

- PS：redis使用的就是hyperloglog（记录当前数据的hash值中第一个1的位置，通过这种标准来记录数据在数据表中有多少个）
...
- 此前说过，join是符合交换律和结合律的，因此如果有多个表进行join，那么就会有很多种不同的执行计划
- 也就是有很多的计划排列组合，那么就需要对其进行剪枝

<br/>

而此前的System R只研究left-deep tree，即只研究左深树
...
<img src="image/multi-relation query planning.png" style="zoom:150%;" />
...
<img src="image/candidate plans_01.png" style="zoom:150%;" />
...
- 不仅仅有左深树，还有右深树，以及二者混合的类型
- 在评估方案的好坏上有两种实现方法：动态规划（如果连表的数量小于12就用dp）和基因遗传算法（如果连表的数量大于12就用GEQO）

<br/>

PG中的遗传算法的主要思路就是每次选取效果最差的那一例剔除，然后将当前最优的那一例分裂出其他的种类（繁衍），以此类推
...
在看了那么多的基于开销的优化模型，一个非常重要的思路便是：最好尽早的将数据过滤

- 数据过滤的越早，后续在算子之间传输的数据就越少
...
动态规划依然是，在做join操作，比较不同join花费时，选取最优策略的最佳手段
