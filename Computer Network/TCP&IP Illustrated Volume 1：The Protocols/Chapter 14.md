...
- TCP拥有两套独立机制来完成重传
  - 一是基于时间（TCP在发送数据时会设置一个计时器，若到计时器超时仍未收到数据确认信息，则会引发相应的超时或基于计时器的重传操作）
  - 二是基于确认信息的构成（若TCP累积确认无法返回新的ACK，或者当ACK包含的SACK表明出现失序报文段时，快速重传会推断出现丢包）
...
### 14.3.1 经典方法（指数加权移动平均法（低通过滤器））

SRTT = α * (SRTT) + (1 - α) * RTT

- SRTT为平滑的RTT估计值
- 新的SRTT是基于现存值SRTT和新的样本值RTT更新的结果，α是平滑因子（推荐值为0.8-0.9）
