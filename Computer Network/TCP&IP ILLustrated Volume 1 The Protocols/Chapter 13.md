...
- 而UDP里面：当一个数据报到达一个不能使用的目的端口时就会产生一个ICMP目的地不可到达的消息
- TCP里面就用重置报文代替
...
- 有序释放：终止一条连接的正常方法是由通信一方发送一个FIN，因为FIN时在之前所有排队数据都已发送后才被发送储区，通常不会出现丢失数据的情况
- 终止释放：任何时刻，发送一个重置报文段替代FIN来终止一条连接
- 终止连接的两大特性：任何排队的数据都将被抛弃，一个重置报文段会被立即发送出去；重置报文段的接收方会说明通信另一方采用了终止的方式而不是依次正常的关闭



13.6.3 半开连接

- 如果在未告知另一端的情况下通信的一段关闭或终止连接，那么就认为该条TCP连接处于半开状态（可能是通信的另一方主机崩溃了）
