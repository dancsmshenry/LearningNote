...
- <img src="image/isolation levels_03.png" style="zoom:150%;" />
...
- <img src="image/isolation levels_05.png" style="zoom:150%;" /> 	
...
- 探讨一下T2对于数据A的读取：
  ...
  - 其实从这里就可以看出单单mvcc是无法实现串行化的（为了不读到脏数据而做出妥协），但是逻辑上却不符合常理，因为要保证T2读到的是T1处理完的数据，而事实却是读到的是T1时最新版本的数据
