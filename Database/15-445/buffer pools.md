...
- 或者说每一个数据库给一个缓存池
- 可以降低锁的冲突（因为缓存池只有一个，往缓存池添加新的数据页的时候，是要加锁的，所以如果分成多个缓存池的时候，可以降低竞争）
...
- 共享扫描，如果两个sql语句都读同一个表，那就可以做同步扫描
  - 两个语句同时到达，需要的结果都是相同的，如果不优化的话，就会分两次对数据进行扫描
  - 有了优化后，就可以共享结果
...
- 即如果大家都需要共同的数据，第一个查询和第二个查询都要查询共同的数据，那么就可以让后面的查询先跟着前面的查询数据，这样就可以提高查询效率，即复用后一个查询的IO顺序，通过共用数据来提高效率
...
- 将获得到的page数据不存储到buffer pool中，因为有些query得到的数据只使用一次，如果放到buffer pool中会导致buffer被污染，换句话说缓存池在这里根本没用
...
- 操作系统对我们的文件页也是有缓存的
- 而我们不需要OS对数据进行缓存，因为我们不知道OS对这些数据会采取什么策略，而且和本地的缓存是冗余冲突的（因为DBMS不知道我们具体的策略是什么）
...
- 即执行引擎给缓存池提供一些信息，让DBMS可以对某些数据页进行保留，可以认为是优先级反转XD
...
- 但是如果很快就要用上的话，那么最好还是把它留在磁盘中
  - 这里会导致持久化的问题，即如果一个数据页一直被使用着，然后又没有刷盘，就会导致持久化的问题
  - 这里可以用log来解决，即WAL
